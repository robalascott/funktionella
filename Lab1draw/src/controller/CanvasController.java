...
public class CanvasController implements Initializable {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		// We don't need selected to be watched since all selected elements
		// should
		// also be in the list of all shapes. With the java.util.observer we
		// would
		...
		// to be watched in a Observable array, we do this in the extractor in
		// the
		// shape, we set a flag for changed, just like setChanged() and calling
		// notifyObservers() with the java.util.observer class. This solution
		// tracks
		// if elements are added and removed to the array aswell, we don't have
		// to
		...
		// If we would have extended this class with Observer instead, we would
		// have
				...
				for (Shape s : shapes) {
					...
					if (s.contains(event.getX(), event.getY())) {
						...
						if (!selected.contains(s)) {
							...
							// We only want to select one shape at a time, and
							// not all
							// the stacked ones
							return;
						}
	...
	/*
	 * public FormattingObject undoAction() { // return factory.undoAction(); }
	 */
	...
	// redraw. TODO: Color is not observed, if we change color the class wont be
	// redrawn
	...
	// we only need one function that just spawn them, otherwise we might want
	// to
